...
export function AppProvider({ children }) {
  ...
  const getLandingPageByProyectoId = async (proyectoId) => {
    try {
      const response = await fetch(`${API_BASE}/landing-pages/by-proyecto/${proyectoId}`, {
        headers: getAuthHeaders()
      });
  ...
  const saveRedactorProgress = async (landingPageId, tableData, annotations = {}) => {
    try {
      ...
      const response = await fetch(`${API_BASE}/secciones-lp/landing-page/${landingPageId}/bulk-update`, {
        method: 'POST',
        headers: getAuthHeaders(),
  ...
  const calculateProgress = (tableData) => {
    ...
    const filledCells = Object.values(tableData).filter(cell => 
  ...
  const loadLandingPageSections = async (landingPageId) => {
    try {
      const response = await fetch(`${API_BASE}/secciones-lp/landing-page/${landingPageId}`, {
        headers: getAuthHeaders()
      });
  ...
  const loadLandingPageAnnotations = async (landingPageId) => {
    try {
      const response = await fetch(`${API_BASE}/anotaciones/landing-page/${landingPageId}/panel`, {
        headers: getAuthHeaders()
      });
      ...
      if (response.ok) {
        ...
        const annotations = {};
        Object.entries(panelData.anotaciones_por_celda).forEach(([cellPosition, anotacionesList]) => {
  ...
  const saveAnnotationToDB = async (landingPageId, cellPosition, text) => {
    try {
      const response = await fetch(`${API_BASE}/anotaciones/landing-page/${landingPageId}/cell`, {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({
          cell_position: cellPosition,
