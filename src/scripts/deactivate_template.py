...
def deactivate_template(template_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("⚠️  Verifica que el ID sea correcto")
            ...
            print("⚠️  Tu API no tiene endpoint PUT para templates")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, template in enumerate(templates, 1):
                ...
                print(f"   🏷️  Proyecto: {template.get('proyecto', 'N/A')}")
...
if __name__ == "__main__":
    ...
    if not active_templates:
        print("\n⚠️  No hay templates activos para desactivar")
    ...
    if not template_info.get('is_active', True):
        print(f"⚠️  El template '{template_info.get('name', template_id)}' ya está inactivo")
        exit()
    ...
    print(f"   🏷️  Proyecto: {template_info.get('proyecto', 'N/A')}")
    
    # Confirmar
    confirm = input(f"\n⚠️  ¿Estás seguro que quieres DESACTIVAR este template? (SI/no): ").strip().upper()
